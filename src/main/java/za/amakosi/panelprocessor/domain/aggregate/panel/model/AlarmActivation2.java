...
@Getter
@AllArgsConstructor
public enum AlarmActivation2 implements Alarm<AlarmActivation2> {
    ...
    public String byAlarm(Integer alarm) {
        var result = Arrays.stream(AlarmActivation2.values()).filter(e -> e.alarm.equals(alarm)).findFirst().orElse(null);
